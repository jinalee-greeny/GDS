...
(function (root) {
  ...
  // Specimen for radius/shadow/space: each role rendered as its applied effect
  // over the semantic light surface, so the set is seen in context.
  function renderSemanticScalarPreview(cfg, grp) {
    var res = C.resolveSemantic(cfg), set = res[grp] || {}, c = res.color.light || {};
    var items = Object.keys(set).map(function (role) {
      var val = set[role], demo;
      if (grp === 'radius') {
        demo = el('div', { class: 'sem-sp-swatch', style: 'border-radius:' + (val || '0') + ';background:' + (c['primary-subtle'] || 'var(--surface-sunken)') + ';border:1px solid ' + (c.border || 'var(--border)') });
      } else if (grp === 'shadow') {
        demo = el('div', { class: 'sem-sp-swatch', style: 'box-shadow:' + (val || 'none') + ';background:' + (c['surface-raised'] || '#fff') + ';border:1px solid ' + (c.border || 'var(--border)') });
      } else { // space
        if (role.indexOf('inset') === 0) {
          demo = el('div', { class: 'sem-sp-inset', style: 'padding:' + (val || '0') + ';background:' + (c['surface-sunken'] || 'var(--surface-sunken)') },
            [el('div', { class: 'sem-sp-inset-fill', style: 'background:' + (c.primary || 'var(--accent)') })]);
        } else {
          demo = el('div', { class: 'sem-sp-gap' }, [
            el('div', { class: 'sem-sp-blk', style: 'background:' + (c.primary || 'var(--accent)') }),
            el('div', { class: 'sem-sp-blk', style: 'margin-left:' + (val || '0') + ';background:' + (c.primary || 'var(--accent)') })
          ]);
        }
      }
      return el('div', { class: 'sem-sp-item' }, [demo, el('span', { class: 'sem-sp-label', text: role })]);
    });
    return el('div', { class: 'sem-sp-frame sem-sp-' + grp, style: 'background:' + (c.bg || '#fff') }, items);
  }

  // Semantic domain view for one module. The domain nav (Color/Text/…) selects
  // the module; each gets a preview column showing its tokens applied in context.
  function renderSemanticView(cfg, moduleKey) {
    ...
    if (moduleKey === 'color') {
      ...
      previewNode = el('div', { class: 'sem-preview-col' }, [
        el('h3', { class: 'sem-pv-heading', text: 'Preview' }),
        renderSemanticScalarPreview(cfg, moduleKey)
      ]);
    }
    var body = el('div', { class: 'sem-layout' }, [previewNode, editorCol]);
